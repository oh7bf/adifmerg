...
[B<-t> min] [B<-p> script] [B<-q> query] [B<-Q> all|requested] [B<-2>] [B<-h>]
[B<-v>] [B<-V>]
...
option 'B<-f> -', it is assumed to be ADIF file. The output is either ADIF, 
csv or simple text. 
...
B<-c> check call sign with dxcc and fill empty CQ/ITU zone and continent 
records
...
The I<adifmerg> has been tested with perl v5.8.5. You may need to adapt 
the script to your environment. 

=head1 BUGS

THROB and PHONE modes are not defined in Adif 2.2.6. See the web page below.
